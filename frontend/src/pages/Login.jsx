...
const Login = () => {
  ...
  const [currState, setcCurrState] = useState("Login");
        ...
        <div className="flexCenter w-full sm:w-1/2">
          <form onSubmit={handleSubmit} className="flex flex-col items-center w-[90%] sm:max-w-md m-auto gap-y-5 text-gray-800">
            ...
            <div className="w-full flex flex-col gap-y-3 medium-14">
              <div className="hover:text-secondary underline cursor-pointer ">
                Forgot your password?
              </div>
              {currState === "Login"?(
                <div>Don't have an account? <span onClick={()=>setcCurrState('Sign Up')} className="underline cursor-pointer  hover:text-secondary">Create account</span></div>
              ):(
                <div>Already have an account? <span onClick={()=>setcCurrState('Login')} className="underline cursor-pointer hover:text-secondary">Login</span></div>
              )}
            </div>
          </form>
        </div>
